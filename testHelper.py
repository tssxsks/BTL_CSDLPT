...
def getopenconnection(user="postgres", password="1234", dbname="ratingsdb"):
...
def getCountrangepartition(ratingstablename, numberofpartitions, openconnection):
    ...
    cur.execute(
        "select count(*) from {0} where rating >= {1} and rating <= {2}".format(
            ratingstablename, 0, interval
        )
    )
    ...
    for i in range(1, numberofpartitions):
        cur.execute(
            "select count(*) from {0} where rating > {1} and rating <= {2}".format(
                ratingstablename, lowerbound, lowerbound + interval
            )
        )
        lowerbound += interval
        countList.append(int(cur.fetchone()[0]))
...
def testrangeandrobinpartitioning(
    n,
    openconnection,
    rangepartitiontableprefix,
    partitionstartindex,
    ...
    with openconnection.cursor() as cur:
        if not isinstance(n, int) or n < 0:
            ...
            count = totalrowsinallpartitions(
                cur, n, rangepartitiontableprefix, partitionstartindex
            )
...
def testEachRangePartition(
    ratingstablename, n, openconnection, rangepartitiontableprefix
):
    ...
    for i in range(0, n):
        ...
        if count != countList[i]:
            raise Exception(
                "{0}{1} has {2} of rows while the correct number should be {3}".format(
                    rangepartitiontableprefix, i, count, countList[i]
                )
            )


def testEachRoundrobinPartition(
    ratingstablename, n, openconnection, roundrobinpartitiontableprefix
):
    countList = getCountroundrobinpartition(ratingstablename, n, openconnection)
    cur = openconnection.cursor()
    for i in range(0, n):
        cur.execute(
            "select count(*) from {0}{1}".format(roundrobinpartitiontableprefix, i)
        )
        count = cur.fetchone()[0]
        if count != countList[i]:
            raise Exception(
...
def testloadratings(
    MyAssignment, ratingstablename, filepath, openconnection, rowsininpfile
):
    """
    ...
    try:
        ...
        with openconnection.cursor() as cur:
            ...
            if count != rowsininpfile:
                raise Exception(
                    "Expected {0} rows, but {1} rows in '{2}' table".format(
                        rowsininpfile, count, ratingstablename
                    )
                )
...
def testroundrobinpartition(
    ...
    try:
        MyAssignment.roundrobinpartition(
            ratingstablename, numberofpartitions, openconnection
        )
        testrangeandrobinpartitioning(
            numberofpartitions,
            openconnection,
        ...
        testEachRoundrobinPartition(
            ratingstablename, numberofpartitions, openconnection, RROBIN_TABLE_PREFIX
        )
    except Exception as e:
        traceback.print_exc()
        return [False, e]
...
def testroundrobininsert(
    ...
    try:
        ...
        if not testrangerobininsert(
            expectedtablename, itemid, openconnection, rating, userid
        ):
            raise Exception(
...
def testrangeinsert(
    ...
    try:
        ...
        if not testrangerobininsert(
            ...
            raise Exception(
                "Range insert failed! Couldnt find ({0}, {1}, {2}) tuple in {3} table".format(
                    userid, itemid, rating, expectedtablename
                )
            )
    except Exception as e:
